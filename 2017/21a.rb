...
input 2017, 21 do |input|
  ...
  5.times do
    ...
    program = grid(program, size) { |chunk|
      _, replacement = rules.find { |pattern, replacement|
        next unless pattern.size == chunk.size

        rotations(pattern, replacement).find { |pattern, replacement|
          pattern == chunk
        }
      }

      raise "couldn't find replacement" unless replacement
