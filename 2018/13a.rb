...
class Car
  ...
  def tick
    ...
    if @network.cars.any? { |car| car.coordinate == next_coordinate } then
      @crash = true
    end
...
class Network
  ...
  def tick
    ...
    crashes = @cars.select { |car| car.crashed?  }

    return true if crashes.empty?
  end
...
##
# I build this to be easy to debug over anything else.  Instead of using
# US-ASCII characters I went ahead with proper box drawing characters with
# arrows for carts in glorious 16-bit color!
#
# Running with printing the network to the screen is very slow, but you can
# see the cars moving about the track which is kind of cute.
#
# Getting all the turn directions correct was tedious, but easier because
# it was symbolic rather than mathematical.  In #turn_at_intersection it was
# nice to be able to use an Enumerator.
#
# I misinterpreted the car ticking and collision instructions so finished this
# and didn't have a working program.  Moving the collision check from after an
# update to during each car's update fixed it for me.
...
input 2018, 13 do |input|
  ...
  crashed = network.cars.find { |car| car.crashed? }

  crashed.coordinate.join ","
end
